...
const CRONOGRAMA_CANTIDAD_HECTAREAS_FIELD = process.env.CRONOGRAMA_CANTIDAD_HECTAREAS_FIELD;
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!CRONOGRAMA_APLICACIONES_TABLE_ID || !CRONOGRAMA_APLICACION_FIELD || !CRONOGRAMA_CANTIDAD_APLICACIONES_FIELD || 
        !CRONOGRAMA_CICLO_DIAS_FIELD || !CRONOGRAMA_CANTIDAD_HECTAREAS_FIELD || !CRONOGRAMA_FECHA_INICIO_FIELD || 
    ...
    if (!APLICACIONES_PROGRAMADAS_TABLE_ID || !APLICACION_MICROORGANISMO_FIELD || !APLICACION_DOSIS_FIELD || 
        !APLICACION_HECTAREAS_FIELD || !APLICACION_FECHA_PROGRAMADA_FIELD) {
    ...
    for (const microSeleccionado of microorganismosSeleccionados) {
      if (microSeleccionado.fechaProgramada) {
        const fechaInicioDate = new Date(fechaInicioAplicaciones);
        const fechaProgramadaDate = new Date(microSeleccionado.fechaProgramada);
        
        if (fechaProgramadaDate < fechaInicioDate) {
          return NextResponse.json({ 
            error: `La fecha programada para ${microSeleccionado.microorganismoNombre} debe ser posterior o igual a la fecha de inicio (${fechaInicioAplicaciones})` 
          }, { status: 400 });
        }
      }
    }

    // Paso 1: Crear las aplicaciones programadas
      ...
      const aplicacionData = {
        records: [{
          fields: {
            ...
            ...(microSeleccionado.fechaProgramada && {
    ...
    const cronogramaData = {
      records: [{
        fields: {
          ...
          [CRONOGRAMA_CANTIDAD_HECTAREAS_FIELD]: parseInt(hectareas),
